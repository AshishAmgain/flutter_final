...
final roomsViewModelProvider = StateNotifierProvider<RoomViewModel, RoomState>((ref) {
  final roomDataSource = ref.read(roomDataSourceProvider);
  return RoomViewModel(roomDataSource);
});

class RoomViewModel extends StateNotifier<RoomState> {
  final RoomDataSource _roomDataSource;

  RoomViewModel(this._roomDataSource) : super(RoomState.initial()) {
    getRooms();
  }
  ...
  Future getRooms() async {
    ...
    if (!hasReachedMax) {
      ...
      result.fold(
        (failure) => state = state.copyWith(hasMaxReached: true, isLoading: false),
        (data) {
          if (data.isEmpty) {
